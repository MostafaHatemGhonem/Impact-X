...
const API = {
    async run_simulation(asteroid_name, diameter, velocity, latitude, longitude, h) {
        if (isFlaskMode) {
            // Flask mode
            ...
            // Pywebview mode
            return await window.pywebview.api.run_simulation(asteroid_name, diameter, velocity, latitude, longitude, h);
        }
    },
    async get_asteroid_list() {
        if (isFlaskMode) {
            // Flask mode
            ...
            return await window.pywebview.api.get_asteroid_list();
        }
    }
};
function TypingEffect(words, target, container) {
...
async function callPythonSimulation(asteroid_name, diameter, velocity, latitude, longitude, h) {
    const containerFiveInputs = document.querySelector(".containerFiveInputs");
    ...
    try {
        ...
        const response = await API.run_simulation(asteroid_name, diameter, velocity, latitude, longitude, h);
        let resultData;
        try {
...
function showScenarioSimulation(scenario) {
    ...
    simulateBtn.addEventListener("click", async () => {
        ...
        try {
            const response = await API.run_simulation(`Save_${scenario.id}`, scenario.diameter, scenario.velocity, latInput.value, lonInput.value);
...
function displaySaveScenarioResults(resultData, scenario, container) {
    container.innerHTML = "";
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        ...
        if (pageOne) {
            const mainTitle = pageOne.querySelector(".maintitle");
            if (mainTitle) {
    ...
    if (pageTwo) {
        const titlePageMain = pageTwo.querySelector(".mainTitlePage");
        if (titlePageMain) {
    ...
    const dystriodVideo = document.querySelector(".dystriodVideo");
